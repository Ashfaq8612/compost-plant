...
jQuery.fn.extend({
    cropzee: function () {
        ...
        if (!settings.allowedExt) {
            settings.allowedExt = ['gif','png','jpg','jpeg'];
        }
        if (!settings.imageExtension) {
            settings.imageExtension = 'image/jpeg';
        }
        if (!settings.returnImageMode) {
            settings.returnImageMode = 'data-url';
        }
        ...
        window.cropzeeCreateImage = function(id) {
            ...
            if (settings.returnImageMode == 'blob') {
                ...
                window.cropzeeReturnImage = [];
                cropzeeReturnImage[id] = canvas.toDataURL(settings.imageExtension);
            }
            /*set image as base64 into input*/
            document.getElementById('file-data').value = canvas.toDataURL(settings.imageExtension);
        ...
        function cropzeeTriggerCroppr() {
            window.cropzeeCroppr = new Croppr('#cropzee-modal-image', {
                ...
                onCropStart: settings.onCropStart,
                onCropMove: settings.onCropMove,
                onCropEnd: settings.onCropEnd,
                onInitialize: settings.onInitialize,
            });
        }
        ...
        function cropzeeTriggerModal(id, src) {
            ...
            var animation = settings.modalAnimation;
            if (animation) {
        ...
        $(this).click(function(){
            ...
            $(this).one("change", function(){
                ...
                if($.inArray(ext, settings.allowedExt) == -1) {
